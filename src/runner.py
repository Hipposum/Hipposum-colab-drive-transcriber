...
Сканирует локальную папку с видео (например, смонтированный Google Drive),
транскрибирует каждое видео с диаризацией спикеров и складывает результаты
в папку work_dir (по одному подкаталогу на видео).

Конфигурация — config/settings.yaml (+ переопределения через env PIPELINE_*).
Секрет — env HF_TOKEN (диаризация pyannote).
"""
...
def main(config_path=None):
    ...
    from .storage import (get_local_video_list, resolve_to_video_files,
    ...
    if LOCAL_FILE and os.path.exists(LOCAL_FILE):
        ...
        pending = [{"local_path": LOCAL_FILE, "name": os.path.basename(LOCAL_FILE),
                    "is_zip": ext in ZIP_EXTENSIONS}]
    elif LOCAL_DIR and os.path.isdir(LOCAL_DIR):
        pending = get_local_video_list(LOCAL_DIR, WORK_DIR, DOWNLOAD_DIR,
                                       MAX_VIDEOS, SKIP_DONE, VIDEO_EXTENSIONS, ZIP_EXTENSIONS)
    else:
        print("local_dir / local_file не задан или путь не найден!")
        pending = []

    # ZIP → плоский список видео
    videos = resolve_to_video_files(pending, DOWNLOAD_DIR, VIDEO_EXTENSIONS, ZIP_EXTENSIONS)
    print(f"\nИтого видео в очереди: {len(videos)}")
    ...
    for i, vf in enumerate(videos, 1):
        ...
        progress["transcribed"].append(vf["name"])
        # Видео из ZIP: помечаем и сам архив, чтобы не распаковывать повторно
        if vf.get("from_zip"):
            progress["transcribed"].append(vf["from_zip"])
        save_progress(progress, progress_file)
        clear_ckpt(WORK_DIR, res["base_name"])

        # Видео, извлечённое из ZIP во временную папку, больше не нужно
        if vf.get("from_zip"):
            try:
    ...
    print(f"\n{'='*65}\nГОТОВО: {n_done} видео | результаты: {WORK_DIR}\n{'='*65}")
