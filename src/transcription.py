...
from .utils import free_gpu, gpu_mem
from .problem_zones import is_hallucination, is_hallucination_segment
...
def transcribe_pass1(audio, config, device, silero_model=None, silero_utils=None):
    ...
    if silero_model is not None and silero_utils is not None:
        ...
        vad_options = {"vad_onset": VAD_ONSET, "vad_offset": VAD_OFFSET}
        print(f"   Используется встроенный pyannote VAD (onset={VAD_ONSET}, offset={VAD_OFFSET})")

    base_device = device.split(":")[0] if ":" in device else device
    device_index = int(device.split(":")[1]) if ":" in device else 0

    model = whisperx.load_model(
        WHISPER_MODEL, base_device, device_index=device_index, compute_type=COMPUTE_TYPE,
